...
安装kubernetes集群并不像Docker Desktop那样简单。官方推荐使用*minikube*配置开发环境。

*minikube*提供很多不同的驱动用以建立集群。这些驱动都可归于3类：虚拟机、容器和裸机。

#### 虚拟机

为在不同的OS上保持统一的体验，最优的虚拟机驱动是virtualbox，因为它在所有主流的OS上都能用。

在中国，Google官方的服务不可用，因此必须配置镜像站点。
...
上面的命令对应的minikube版本是1.11.0。如使用其他版本，更改iso的版本即可。

#### 容器PID

另一个全平台通用的驱动是docker。还是一样，在中国使用必须采取一些配置：
...
这个集群由一个容器中的docker管理。当docker hub不可用时，必须使用代理拉取镜像。[这个博客](https://blog.csdn.net/activity110/article/details/85241450)提供一个简单的方法。

#### 裸机

这个配置只能在Linux上使用，而且可能造成宿主机的数据损失，因此不推荐采用。如果这些风险可以接受，可以用以下命令在中国启动一个裸机集群：
...
另一个难点是在宿主机和容器间分享文件。因为容器跑在虚拟机上，这尤其困难。

有个好消息是，当使用VirtualBox作为VM驱动时，minikube自动将`$HOME`目录连接到虚拟机中。这让连接虚拟机的文件系统和集群成为可能。kubernetes提供local storage和persistent volume达成这个目的。我做了一个[案例配置](https://github.com/onichandame/docker-dev/blob/master/kube/dev.yaml)。

注意这个功能有一个缺陷：无法成功创建软链接。可以在[这个issue](https://github.com/kubernetes/minikube/issues/4621)中追踪进度。
