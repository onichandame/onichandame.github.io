...
在命名空间`ambassador`种的所有 pod 都上线后，控制器即为可用状态。

另一个需要注意的点是，所有需要使用 ambassador 的 ingress 都需要标注为`kubernetes.io/ingress.class: ambassador`，以通知 ambassador 控制器暴露这个 ingress。
