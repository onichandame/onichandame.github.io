...
Setting up a kubernetes cluster is not as straightforward as running Docker Desktop. The official way to setup a development cluster is [minikube][minikube]. But I choose to use [kind][kind] as it is more consistent across different platforms than [minikube][minikube].
...
Another caveat currently not resolved is the that the coredns service may stop resolving domain names after the host being restarted. This can be worked around by restarting the coredns deployment on host restart.
