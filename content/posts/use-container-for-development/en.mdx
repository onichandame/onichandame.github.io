...
As development environment requires a lot of tools to be installed, the image is inevitably bloated. My image based on CentOS weights more than 3GB. Therefore it is necessary to reduce the image size.

The best way to reduce image size is rebasing the image to alpine or busybox. However, one of my project requries glibc and CentOS. I cannot shift my development to alpine.

Another approach is to merge the steps. This approach cannot reduce the image size by a lot, but it has another benefit that programmatically run the setup process using `bash` or `sh`.
...
Minikube runs a kubernetes cluster in a virtual machine. To make sure the consistency across different OS, virtualbox is the best VM driver in this case.

Another problem is that the internal image of kubernetes are hosted by Google, which is not accessible in China. Minikube allows to use a mirror registry to bypass this barrier. In short, the final command I use is:
...
Another tricky setup is to share files between the container and the host. This is difficult as the containers run in virtual machines.

The good news is that minikube automatically maps the `$HOME` directory to the virtual machine when using virtualbox as the VM driver. This allows us to create local storage and persistent volume that connects the virtual machine's filesystem to the cluster. The example configuration can be seen in [this file](https://github.com/onichandame/docker-dev/blob/master/kube/dev.yaml)
