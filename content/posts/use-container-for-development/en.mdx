...
Setting up a kubernetes cluster is not as straightforward as running Docker Desktop. The official way to setup a development cluster is *minikube*.

*minikube* provides many different drivers to setup a cluster, all boils down to 3 categories: virtual machine, container and bare metal.

#### Virtual Machine

To make sure the consistency across different OS, virtualbox is the best VM driver as it works on all major OS.

To run a virtualbox backed cluster in China, mirror sites should be used as the official google services are blocked by the censorship.

```bash
minikube start --driver=virtualbox --image-repository=registry.cn-hangzhou.aliyuncs.com/google_containers --registry-mirror=https://reg-mirror.qiniu.com --iso-url=https://kubernetes.oss-cn-hangzhou.aliyuncs.com/minikube/iso/minikube-v1.11.0.iso --memory=12G --cpus=3
```

The command above is for minikube ver. 1.11.0. For other versions change the iso version to match your choice.

#### Container

Another driver that stays consistent across different OS is docker. Again, in China some tweaks are necessary:
...
The cluster uses docker inside a container to orchestrate the resources. In case the docker hub is unavailable, the docker should be configured to use a proxy for pulling images. [This blog](https://blog.csdn.net/activity110/article/details/85241450) provides an easy way to tunnel the docker daemon through a socks proxy.

#### Bare Metal

This setup is not recommended as it only works on Linux and may cause data loss on the host machine as it runs all k8s components directly on the host. If the risks are tolerable, the command below may be used to start a bare metal cluster in China:

```bash
sudo minikube start --driver=none --image-repository=registry.cn-hangzhou.aliyuncs.com/google_containers --registry-mirror=https://reg-mirror.qiniu.com
```

### Persistent Volume

Another challenge is how to store the files persistently. The virtual machines or containers can be killed and removed at any time, so the persistent files have to reside in the host filesystem. *minikube* provides a mounting utility that can mount any directory on the host to the virtual machine or container.

```bash
minikube mount /home/user/files:/files
```

Note there is one caveat of the mounting utility. Creating symlinks will fail. Check [this issue](https://github.com/kubernetes/minikube/issues/4621) for progress.
