...
因为开发环境需要很多工具，镜像的大小不可避免地会失控。我的基于CentOS的镜像有超过3GB，因此需要尽量缩减。

最优的办法是使用alpine或busybox重构镜像。但是我负责的一个项目需要glibc和CentOS，因此我不能采用alpine。

另一个可行的办法是减少中间镜像。这个方法不能显著减少镜像大小，但它还有助于使用`bash`或`sh`程序化地配置环境。
...
Minikube在虚拟机中配置一个集群。考虑到不同OS上的通用性，VirtualBox是最好的选择。

另一个问题是很多内部的镜像都在Google自己的平台上，国内无法直接连接。Minikube允许使用镜像站点绕过这个限制。简单来说，我最终使用如下命令：
